...
#### ● Windows系统双击bilitool.exe没反应
windows系统右击压缩包，属性 → 勾选“解除锁定”，然后再解压缩运行

#### ● MacOS运行提示"已损坏，无法打开。你应该将它移到废纸篓"
MacOS默认不允许运行非官方签名的程序，解决办法：
...
2.在终端中输入`sudo xattr -cr ` (这里要注意后面有个空格)
拖入命令行后， 拖入程序图标后类似显示`sudo xattr -cr /Applications/XXX.app`，然后回车，再去打开程序即可正常运行
